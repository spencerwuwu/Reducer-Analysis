...
import statementResolver.soot.StatementResolver.MemModel;
import statementResolver.soot.SootRunner;
...
//			}
	...
	public SourceLocation getSourceLocation(SootMethod sm) {
		LineNumberTag tag = (LineNumberTag) sm.getTag("LineNumberTag");
		int lineNumber = -1;
		if(tag != null)
			lineNumber = tag.getLineNumber();
	...
	public int getJavaSourceLine(AbstractHost ah) {
		LineNumberTag tag = (LineNumberTag) ah.getTag("LineNumberTag");
		int lineNumber = -1;
		if(tag != null)
			lineNumber = tag.getLineNumber();
		return lineNumber;
